...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--width", type=int, default=4096, help="output width; height is half (default 4096)")
    ...
    p.add_argument("--source", default="source_4k.exr", help="source file name inside env_dir")
    ...
    if args.cmd == "condition":
        print(json.dumps(cook.condition(args.src, args.dst, width=args.width), indent=2))
        return 0
    if args.cmd == "cook":
        manifest = cook.cook_environment(
            args.env_dir, base=args.base, samples=args.samples, backend=args.backend, source=args.source
        )
